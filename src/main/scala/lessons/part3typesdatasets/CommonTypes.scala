...
object CommonTypes extends App {
  ...
  // math operators
  val ratingCalc =
    ((col("Rotten_Tomatoes_Rating") / 10 + col("IMDB_Rating")) / 2).as("rating")
  moviesDF
    .select(col("Title"), ratingCalc)
    .where(col("rating").isNotNull)
    .show(false)

  // correlation = number between -1 and 1
  val correlation = moviesDF.stat.corr("Rotten_Tomatoes_Rating", "IMDB_Rating")
  println(s"Correlation between Rotten_Tomatoes_Rating and IMDB_Rating is $correlation")
  ...
  carsDF
    .select(
      col("Name"),
      regexp_extract(col("Name"), getCarNamesRegex, 0).as("regex_extract"),
    )
    .where(col("regex_extract") =!= "")
    .drop("regex_extract")
  ...
  carsDF.filter(bigFilter).show(false)
